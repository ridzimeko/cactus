    ...
    <article class="group flex gap-6">
        <div class="flex-shrink-0">
            <a class="text-base hover:underline" href="{{ route('profile.view', $question->user->username) }}">
                <x-user-avatar :avatar="$question->user->profile_img" class="flex-shrink-0" size="small" />
            </a>
        </div>
        ...
        <div class="w-full text-white">
            <header class="flex items-center gap-2">
                <a class="text-lg text-gray-200 hover:underline"
                    href="{{ route('profile.view', [$question->user->username]) }}">
                    {{ $question->user->name }}
                </a>
                <span class="text-gray-400">{{ '@' . $question->user->username }}</span>
                <span class="text-base text-gray-400">{{ $this->getformattedTime($question->updated_at) }}</span>
            ...
            <a href="{{ route('question.view', ['question_id' => $question->id]) }}"
                wire:navigate>{{ $question->answers_count ?? 0 }}
                jawaban</a>
        </div>
    </article>
</div>
